...
export default function EditProfileView({navigation}) {
  ...
  const id = useSelector(state => state.auth.data.users.id);
  const data = useSelector(state => state.editprofile);
  const [fullname, setFullname] = useState(auth.data.users.username);
  const [email, setEmail] = useState(auth.data.users.email);
  const [posted, setPosted] = useState(false);
  const [filePath, setFilePath] = useState(auth.data.users.photo);
